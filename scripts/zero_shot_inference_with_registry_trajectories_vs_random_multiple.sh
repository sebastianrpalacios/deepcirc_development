...
for ID in "${IDS[@]}"; do
  OUTPUT_FOLDER="/home/gridsan/spalacios/Designing complex biological circuits with deep neural networks/manuscript/zero-shot/against_random/GAT_MLP_with_scalars/initial_state_sampling_factor_0/4_inputs_${ID}_samplingfactor_0_4000nn/seed_1"
  mkdir -p "${OUTPUT_FOLDER}"

  (
    # Per-ID logging, same style as your original
    exec > >(tee -a "$OUTPUT_FOLDER/slurm-${SLURM_JOB_ID}.log") 2>&1

    python zero_shot_inference_with_registry_trajectories_vs_random.py \
