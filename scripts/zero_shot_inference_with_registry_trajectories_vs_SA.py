...
# SA track: BACKTRACKING env (adds one extra action)
...
def _get_backtrack_index(env) -> Optional[int]:
    """
    Try to discover the backtrack action index from the underlying env.
    If unavailable, return None; SA will still run but cannot revert on reject.
    """
    for name in ("get_backtrack_action", "backtrack_action_index", "backtrack_action"):
        try:
...
def _current_energy_from_info_or_env(env, info) -> Optional[float]:
    # 1) info dict
    try:
        if isinstance(info, dict) and "energy" in info and info["energy"] is not None:
            return float(info["energy"])
    except Exception:
        pass
    # 2) last logged trajectory step
    try:
        get_traj = env.get_wrapper_attr("get_trajectory")
        if get_traj:
            traj = get_traj()
            if traj and isinstance(traj[-1], dict) and "energy" in traj[-1]:
                return float(traj[-1]["energy"])
    except Exception:
        pass
    # 3) try direct attribute
    try:
        val = env.get_wrapper_attr("current_energy")
        if val is not None:
            return float(val)
    except Exception:
        pass
    try:
        return float(getattr(env.unwrapped, "current_energy", None))
    except Exception:
        return None
...
def run_episode_sa_masked(
    ...
    """
    Simulated Annealing over masked actions.
    - Propose a valid action.
    - Move; compute ΔE (prefer 'energy' from info or trajectory).
    - Accept if ΔE <= 0; else accept with probability exp(-ΔE / T).
    - If rejected and a backtrack action is available now, immediately backtrack.

    Notes:
    - If energy is unavailable, uses (negative reward) as proxy for ΔE (i.e., larger reward => lower energy).
    - If backtrack index cannot be discovered, rejections cannot be reverted (the move remains).
    """
    # reset
    try:
    ...
    # attempt to get starting energy
    prev_energy = None
    prev_reward = 0.0

    # discover backtrack action index (if available)
    ...
    # for cooling
    # if total_budget_steps not provided, estimate from env if possible, else 1e6 to simulate slow cooling
    if total_budget_steps is None:
        total_budget_steps = getattr(env.unwrapped, "max_steps", None) or 1_000_000

    while not done:
        T = _temperature(step_idx, total_budget_steps, t0, tmin, schedule)
        ...
        # Choose a proposal. Optionally avoid proposing backtrack unless it's the only option.
        cand = valid
        ...
        if new_energy is not None:
            if prev_energy is None:
                # first measured energy, initialize
            ...
            # Fallback: use reward sign (maximize reward ≈ minimize energy)
            if reward >= prev_reward:
                accept = True
                prev_reward = reward
            else:
                p = math.exp((reward - prev_reward) / max(T, 1e-12))
                if rng.random() < p:
                    accept = True
                    prev_reward = reward
                else:
        ...
        # If rejected and we can backtrack now, try to revert immediately
        if (not accept) and (not done) and (back_idx is not None):
            # after the proposal step, check mask again and backtrack if allowed
            ...
            if back_idx in valid2:
                ...
                if verbose:
                    print(f"[SA] Rejected proposal -> BACKTRACK (action {back_idx}) at step {step_idx}", flush=True)
...
def build_env_sa_store_only(seed_graphs, max_nodes, max_steps, existing_keys,
    ...
    base_env = DRL3envBacktrack(
        ...
        registry_lock=registry_lock,
        store_every_new_graph=True,
        sampling_from_shared_registry=False,  # do NOT sample for SA track
        registry_read_only=False,
        max_steps=max_steps,
        enable_full_graph_replacement=True,
        show_plots=False,
...
# ----------------------- Main -----------------------
def main():
    p = argparse.ArgumentParser("Trained(masked) + SA(masked: 1 long trajectory)")
    ...
    p.add_argument("--sa_verbose", action="store_true", default=False, help="Verbose SA logs")
    args = p.parse_args()
    ...
    # ========= SA (masked, ONE long trajectory = episodes * max_steps; store-only) =========
    ...
    # Set the SA env's max_steps to the full budget
    sa_budget_steps = int(args.episodes) * int(args.max_steps)
    ...
    # Run ONE long SA episode
    print(f"[INFO] SA single-trajectory budget = {sa_budget_steps} steps "
    ...
    csv_path_sa = out_sa / "sa_episode_metrics.csv"
    ...
    save_registry_pickle(registry2, lock2, out_sa / "sa_final_shared_registry.pkl")
    save_registry_summary_csv(registry2, lock2, out_sa / "sa_registry_summary.csv")
