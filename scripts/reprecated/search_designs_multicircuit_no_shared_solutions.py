...
#----------------------------DEBUG CODE TO CATCH SIMPLEX ISSUE-----------------------
'''
import torch
from torch.distributions import Categorical, utils as dist_utils
from sb3_contrib.common.maskable.distributions import MaskableCategorical

def debugged_apply_masking(self, masks):
    # ------------------------------------------------------------------ #
    # 1) build masked logits exactly as SB3-contrib does
    if masks is not None:
        device = self._original_logits.device          # robust choice
        ...
        HUGE_NEG = torch.tensor(-1e8,
                                dtype=self._original_logits.dtype,
                                device=device)
    ...
    # ------------------------------------------------------------------ #
    # 2) quick pre-check – print if (sum-1) outside 1e-6 band
    with torch.no_grad():
        # do *exactly* the normalisation PyTorch applies internally
        logits_norm = logits - logits.logsumexp(dim=-1, keepdim=True)
        probs       = torch.softmax(logits_norm, dim=-1)
        row_sums    = probs.sum(dim=1)
        bad         = (row_sums < 1 - 1e-6) | (row_sums > 1 + 1e-6)
        if bad.any():
            idx = bad.nonzero(as_tuple=True)[0][:5].tolist()
            print("⚠️  SIMPLEX VIOLATION detected **before** calling PyTorch")
            print("   rows      :", idx)
            print("   row-sums  :", row_sums[bad][:5].cpu().numpy())
            print("   min(prob) :", probs[bad].min().item())
            print("   sample logits:", logits[bad][0][:10].cpu().numpy())
            print("   sample probs :",  probs[bad][0][:10].cpu().numpy(),
                  flush=True)      # ensure order in stdout
            # <- do **not** raise here; let PyTorch throw its own error
    # ------------------------------------------------------------------ #
    # 3) now call the real Categorical ctor – if still invalid,
    #    torch.distributions will raise the standard ValueError.
    Categorical.__init__(self, logits=logits)

    # keep .probs consistent for downstream code
    self.probs = dist_utils.logits_to_probs(self.logits, is_binary=False)


# Install the patch
MaskableCategorical.apply_masking = debugged_apply_masking
'''
...
class RegistrySnapshotCallback(BaseCallback):
    def __init__(self, registry, lock=None, every_rollouts=10,
                 dir_path="runs", name="registry", verbose=0):
...
if __name__ == '__main__':
    ...
    folder_name = "runs/20250528_1_multiinput_not_pretrained_no_registry_env4_10_boolean_functions_GPU"
    total_timesteps = 1600*60
    n_epochs = 4
    batch_size = 160
    ...
    model.learn(total_timesteps , callback=[per_env_cb, progress_cb, checkpoint_cb, snap_cb], progress_bar=False)
    ...
    # Save the model
    #model.save("20250427_1_model_all_envs_0x3AC7_designs_topology_design_batch_0_0_entropy_GPU_prunedgraphs_80workers")
