...
def debugged_apply_masking(self, masks):
    if masks is not None:
        ...
        self.masks = torch.as_tensor(masks, dtype=torch.bool, device=device).reshape(self._original_logits.shape)
        HUGE_NEG = torch.tensor(-1e8, dtype=self._original_logits.dtype, device=device)

        logits = torch.where(self.masks, self._original_logits, HUGE_NEG)

        # ✅ Normalize logits *exactly* as PyTorch does internally
        ...
        if bad.any():
            indices = bad.nonzero(as_tuple=True)[0].tolist()
            print("⚠️ Simplex violation BEFORE PyTorch check!")
            print("Invalid row indices:", indices[:5])
            print("Row sums:", row_sums[bad][:5].cpu().numpy())
            print("Logits sample:", logits[bad][0][:10].cpu().numpy())
            print("Probs sample:", probs[bad][0][:10].cpu().numpy())
            raise ValueError("Simplex constraint violated pre-Categorical")

    else:
        self.masks = None
        logits = self._original_logits

    # Let PyTorch handle everything now
...
if __name__ == '__main__':
    ...
    model.learn(total_timesteps , tensorboard_log="./", progress_bar=False)
