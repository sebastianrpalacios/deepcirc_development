...
if __name__ == '__main__':
    ...
    parser.add_argument("--target_hex", type=str, help="Hex identifier of circuit to train on, e.g. 0x3AC7")    
    ...
    best_value  = manager.Value('d', math.inf)
    ...
    # ---------- Keep *only* target_hex circuit ---------------------------------------------
    pattern    = f"{target_hex}_NIG_unoptimized.pkl"

    all_circuit_files = glob.glob(
        os.path.join(directory_path, pattern)
    )

    if not all_circuit_files:
        raise RuntimeError(f"No circuit file found for {target_hex}!")

    # We know there is exactly one, so skip random sampling
    selected_circuit_files = all_circuit_files    # one-element list
    n = 1                                         # override CLI if needed

    print(f"[info] 1 circuit selected: {all_circuit_files[0]}", flush=True)
    # --------------------------------------------------------------------------- 
    ...
    def make_env():
        ...
        return DRL3env(
            ...
            best_energy_across_workers = best_value,
        )
    '''
    def make_env():
        # any per-worker initialisation
