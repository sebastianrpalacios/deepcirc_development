...
def cello2_toxicity_score(gate_name, cumulative_input, gate_toxicity_df):
...
def is_roadblocking(graph, roadblocking_inputs =[0, 2]):
    ...
    valid_nodes = []
    specific_repressors = {'PhlF', 'SrpR', 'BM3R1', 'QacR'}
    found = False
...
def draw_network_with_colors_and_labels_from_G(G):
    color_mapping = {
        'AmeR': 'lightblue',
        'HlyIIR': 'lightgreen',
        'PhlF': 'orange',
...
def rename_nodes_by_mapping_input_nodes(dag_to_rename, mapping_source, mapping_target):
    ...
    # Step 1: Identify nodes with both incoming and outgoing edges
    nodes_with_incoming_edges = [node for node in dag_to_rename.nodes() if dag_to_rename.in_degree(node) > 0]

    # Step 2: Create a mapping for nodes to add a '*' for identified nodes, and directly map others as per original mapping
    intermediate_mapping = {}
    for node in dag_to_rename.nodes():
        if node in nodes_with_incoming_edges:
            # Append '*' to node IDs that have both incoming and outgoing edges
            intermediate_mapping[node] = f"{node}*"
        else:
            # For nodes without both in and out edges, check if they are in the original mapping target list
...
def rename_nodes_by_mapping_exit_points(dag_to_rename, mapping_source):
    ...
    # Assuming a direct mapping based on order, which might need adjustments based on your specific requirements
...
def substitute_subgraph(G, subgraph, replacement_graph, draw_relabeling = False):
    ...
    for cut_node in subgraph_input_nodes:
        for exit_node in subgraph_output_nodes:
            if G_copy.has_edge(cut_node, exit_node):  # Check if the edge exists before attempting to remove it
                G_copy.remove_edge(cut_node, exit_node)
...
# ### Function and general variable definitions

# Generate and save permutations without repeated repressors
...
#elements = list(range(20))  # Elements to choose from
#r = 5  # Number of elements to choose
...
# Type alias for the implicit‑OR detector
ImplicitORFunc = Callable[[nx.DiGraph, Any, int], Dict[Any, Dict[str, Any]]]

def energy_score(
    G: nx.DiGraph,
    implicit_or_fn: ImplicitORFunc,
    *,
    fanin_size: int = 2,
    ...
    for key, val in implicit_or_results.items():
        if val.get("is_there_an_implicit_OR", False) and \
           val.get("number_of_nodes_available_for_removal", 0) > max_removal:
