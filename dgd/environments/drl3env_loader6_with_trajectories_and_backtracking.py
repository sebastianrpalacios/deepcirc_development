...
class DRL3env(gym.Env):
    ...
    def __init__(
        ...
        *,
        ...
        self.BACKTRACK_ID = self.TERMINATE_ID + 1       
        self.action_space = spaces.Discrete(self.TERMINATE_ID + 2)
        ...
        self._trajectory = []  # list of per-step snapshots for current episode  
        
        # stack of graph snapshots (depth 0 = post-reset)
        self._state_stack = []      

    #---Clases for backtracking---
    @staticmethod
    def _copy_graph(g):
        try:
            return g.copy()
    ...
    def _snapshot_current_graph(self):
        """Capture a copy of the current graph into the backtrack stack."""
        G = getattr(self, "current_solution", None)
        if G is None:
            raise RuntimeError("Env has no 'current_solution' to snapshot.")
        self._state_stack.append(self._copy_graph(G))

    def _restore_graph_only(self, graph):
        """Restore ONLY the graph and clear caches so masks recompute correctly."""
        self.current_solution = self._copy_graph(graph)

        # Clear caches so action_masks recomputes for the restored graph
        self._mask_cache = None
        if not hasattr(self, "_subgraphs_for_action") or self._subgraphs_for_action is None:
            self._subgraphs_for_action = defaultdict(list)
        else:
    ...
    def start_trajectory_logging(self, enable: bool = True):
        """Enable/disable per-episode trajectory logging."""
        self.enable_trajectory_logging = bool(enable)

    def get_trajectory(self):
        """Return the current episode trajectory snapshots (list of dicts)."""
    ...
    def reset(self, *, seed=None, options=None):
        ...
        #Add initial graph to stack
        self._snapshot_current_graph()
        
        
        # Trajectory logging: start new log and add initial snapshot (step 0)
        if self.enable_trajectory_logging:
            self._trajectory = []
            try:
                graph_json = nx.node_link_data(self.current_solution)
            except Exception:
                graph_json = None
            self._trajectory.append({
                ...
                "repl": None,
                "graph": graph_json,
                "energy": float(self._best_energy_in_episode),
            })
    ...
    def action_masks(self):
        ...
        # ========= NEW: compose final mask with BACKTRACK and TERMINATE rules =========
        depth = len(self._state_stack) - 1  # 0 at root
        has_forward = bool(mask[: self.TERMINATE_ID].any())
        has_terminate = bool(mask[self.TERMINATE_ID])

        # Expand to include BACKTRACK slot
        mask_full = np.zeros(self.TERMINATE_ID + 2, dtype=bool)  # + BACKTRACK
        mask_full[: self.TERMINATE_ID + 1] = mask

        # Default: hide TERMINATE from random choice
        if has_terminate:
            mask_full[self.TERMINATE_ID] = False

        # Offer BACKTRACK if we can
        if depth > 0:
            mask_full[self.BACKTRACK_ID] = True

        
        # If there are NO forward actions…
        if not has_forward:
            if has_terminate:
                # Only TERMINATE was available
                if depth > 0:
                    # Prefer BACKTRACK over TERMINATE (don’t allow terminate here)
                    mask_full[self.BACKTRACK_ID] = True
                    mask_full[self.TERMINATE_ID] = False
                else:
                    # At root: allow TERMINATE to end the episode
                    mask_full[self.TERMINATE_ID] = True
                    mask_full[self.BACKTRACK_ID] = False
            #else:
            #    # Truly nothing: only BACKTRACK if possible
            #    mask_full[self.BACKTRACK_ID] = (depth > 0)
        

        self._mask_cache = mask_full
    ...
    def step(self, action):
        ...
        # ========= NEW: handle BACKTRACK action =========
        if int(action) == int(self.BACKTRACK_ID):
            depth = len(self._state_stack) - 1
            if depth <= 0:
                # At root: nothing to backtrack to. No-op graph change, still counts as a step.
            ...
            # Pop current snapshot and restore previous graph
            self._state_stack.pop()
            prev_graph = self._state_stack[-1]
            self._restore_graph_only(prev_graph)

            # Keep best-energy definitions unchanged
            reward  = (100.0 / self._best_energy_in_episode) if (done and self._best_energy_in_episode != 0) else 0.0
            ...
            if self.enable_trajectory_logging:
                try:
                    ...
                    graph_json = None
                self._trajectory.append({
                    "step": int(self.current_step_in_episode),   # already incremented
                    "action_id": int(self.BACKTRACK_ID),
                    "tgt": None, "cut": None, "cand": None, "repl": None,
                    "graph": graph_json,
                    "energy": float(self._best_energy_in_episode),
                    "event": "backtrack",
                })
        ...
        # Trajectory log: snapshot after applying rewrite
        if self.enable_trajectory_logging:
            try:
                cand_json = nx.node_link_data(cand)
            except Exception:
                cand_json = None
            try:
                repl_json = nx.node_link_data(repl)
            except Exception:
                repl_json = None
            try:
                graph_json = nx.node_link_data(self.current_solution)
            ...
            self._trajectory.append({
                ...
                "tgt": int(tgt) if isinstance(tgt, (int, np.integer)) else tgt,
                "cut": [int(x) for x in cut] if hasattr(cut, "__iter__") else cut,
                "cand": cand_json,
                "repl": repl_json,
                "graph": graph_json,
        ...
        # ========= NEW: snapshot post-rewrite so we can backtrack to this state later =========
        if not done:
            self._snapshot_current_graph()
        # ========= END NEW =========   
