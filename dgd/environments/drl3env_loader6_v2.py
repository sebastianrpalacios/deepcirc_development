...
def _apply_implicit_or_using_factor_cuts(G, fanin_size: int = 2):
    G_copy = G.copy()
...
def _canonical_graph_transform(g):
    c1 = _apply_implicit_or_using_nor_not_motif(g.copy())
    c2 = _apply_implicit_or_using_factor_cuts(g.copy())
    e1 = energy_score_general(c1)[0]
    e2 = energy_score_general(c2)[0]
    return c1 if e1 <= e2 else c2     

def _canon_and_energy(g: nx.DiGraph):
    c = _canonical_graph_transform(g)
    e, _ = energy_score_general(c)
    return e, c
...
class DRL3env(gym.Env):
        ...
        *,
        ...
        if self._shared_registry is not None:
            ...
            if self.sampling_from_shared_registry:
                ...
                self._seed_energies = [_canon_and_energy(g)[0] for g in self.initial_states]            
        ...
        with (self._registry_lock):
            ...
            if len(self._shared_registry):                
                ...
                for g_seed in self.initial_states:                    
                    canon = _canonical_graph_transform(g_seed)
                    ...
                    bucket.append((canon, g_seed.copy(), energy_score_general(canon)[0]))
    ...
    def reset(self, *, seed=None, options=None):
        ...
        current_solution_canonical = _canonical_graph_transform(self.current_solution)
        self._best_energy_in_episode, _ = energy_score_general(current_solution_canonical)
    ...
    def step(self, action):
        ...
        current_solution_canonical = _canonical_graph_transform(self.current_solution)
        current_energy, _ = energy_score_general(current_solution_canonical)        
        ...
        # Case B: shared registry used 
             ...
             with (self._registry_lock):
                ...
                if run_store_graph_routine_flag:              
                    ...
                    canon = _canonical_graph_transform(self.current_solution)
                    h     = _compute_hash(canon)
